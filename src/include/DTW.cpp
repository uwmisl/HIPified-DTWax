...
// Cost = cost_here + min(left, top, diag)
#ifndef NO_REF_DEL
...
// and the reference values are ref_coeff1[]. To compute this, it is given
                ...
                val_t (&penalty_temp)[2], idxt query_batch)
{
  ...
  if ((thread_id != (wave - 1)) || (query_batch))
  {
    ...
    penalty_here[SEGMENT_SIZE - 1] =
        COST_FUNCTION(query_val, ref_coeff1[SEGMENT_SIZE - 1], penalty_here[SEGMENT_SIZE - 2],
                      penalty_here[SEGMENT_SIZE - 1], penalty_temp[0]);
#endif
  }

  // Else, query_batch==0 and thread_id==(wave-1)
  // I think this is the first or last row??
  else
  {
#ifdef HIP_DEBUG
#ifndef FP16
...
#pragma unroll
  for (idxt query_batch = 0; query_batch < QUERY_BATCH; query_batch++)
  {
    ...
    val_t penalty_here[SEGMENT_SIZE] = {FLOAT2HALF(0)}; // MQ: I think this is where I adjust to be INFINITY instead of 0 to do global rather than subsequence alignment
...
#pragma unroll
    for (idxt i = 0; i < SEGMENT_SIZE; i++)
    {
      ...
      // I think this is un-shuffling that?
    ...
    for (idxt wave = 1; wave <= NUM_WAVES; wave++)
    {
      ...
      last_col_penalty_shuffled =
          __shfl_down(last_col_penalty_shuffled, 1);
